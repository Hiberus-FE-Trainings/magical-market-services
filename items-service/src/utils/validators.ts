...
const isValidKey = (key: string): key is keyof Item => ITEM_KEYS.includes(key as keyof Item);
...
export const validateItemFromRequest = (itemFromRequest: Partial<Item>) => {
  ...
  if (itemFromRequestEntries.length === 0) throw new Error("No item data to update");
